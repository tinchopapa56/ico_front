...
export default function Home(){
  ...
  // Muestra la CANTIDAD de tokens q pueden CLAIMEARSE
  // segun los CD nfts que tienen (q entitles them a unas monedas)
  const [tokensToBeClaimed, setTokensToBeClaimed] = useState(zero);
  //la cantidad de tokensERC20 q tiene en su wallet
  const [balanceOfCryptoDevTokens, setBalanceOfCryptoDevTokens] = useState(zero);
  // The amount que quiere mintear
  const [tokenAmount, setTokenAmount] = useState(zero);
  // los tokens q fueron minteados del total max supply
  const [tokensMinted, setTokensMinted] = useState(zero);
  // si la addres q aparecio es la del Owner
  const [isOwner, setIsOwner] = useState(false);
  // Create a reference que queda SIEMPRE en la pagina, sin importar los renderizados y etc.
